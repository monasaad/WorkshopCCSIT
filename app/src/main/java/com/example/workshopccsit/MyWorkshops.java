...
public class MyWorkshops extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        // Toast.makeText(this.getActivity(), Stu_Id, Toast.LENGTH_LONG).show();


       myDB = new DBHelper(this.getActivity());


        Cursor result2 = myDB.View_Reg_S(S_ID);

        if(result2.getCount() == 0)
        {
            ShowMsg("Hold on!", Stu_name + " you're not registered in any workshop yet.");
        }
        else{
            ...
            adapter = new S_adapter(this,items,S_ID,Stu_name);
